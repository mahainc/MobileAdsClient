...
//
    ...
    final internal class NativeAdManager: NSObject, @unchecked Sendable {
        ...
        private let queue = DispatchQueue(
            label: "com.app.NativeAdManager.\(UUID().uuidString)",
    ...
    extension NativeAdManager: NativeAdLoaderDelegate {
        ...
        public func adLoader(
            ...
            #if DEBUG
                print(
                    "⛔️ NativeAdManager didFailToReceive unit=\(adLoader.adUnitID) error=\(error.localizedDescription)"
                )
            #endif
            queue.async {
                if let (id, _) = self.pendingRequests.first(where: { $0.value.adLoader === adLoader }) {
