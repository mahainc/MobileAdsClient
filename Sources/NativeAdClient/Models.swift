...
extension NativeAdClient {
    public enum Configuration {
        ...
        public struct Metrics: Sendable, Equatable, Hashable {
            ...
            // Narrow grid-cell poster: small icon, tighter spacing, and a
            // rounder container so the 9:16 media hero reads as a card.
            public static let portrait = Metrics(
        ...
        // Portrait "poster" card for grid cells: a fixed 9:16 media hero
        // pinned flush to the card's top + sides, with a compact
        // icon / headline / advertiser column + full-width CTA below. Unlike
        // `RowMedia`, the media box is LOCKED to `mediaAspectMultiplier` and
        // is NOT rewritten to the creative's true ratio, so every card in a
        // grid keeps a uniform hero height.
        public struct Portrait: Kind, Hashable {
            public var style: Style
            public var bodyDisplay: BodyDisplay
            /// Outer padding around the CTA below the media. Left/right default
            /// to 0 so the CTA spans the full media width; only `bottom` adds a
            /// trailing gap. The media hero is always flush to the card's edges.
            public var insets: UIEdgeInsets
            public var metrics: Metrics
            /// Media height = width × this. `16/9` ≈ 1.777… gives a portrait
            /// 9:16 hero (taller than wide). Held fixed for the card's life.
            public var mediaAspectMultiplier: CGFloat

            public init(
                style: Style = .row,
                bodyDisplay: BodyDisplay = .hidden,
                insets: UIEdgeInsets = .init(top: 0, left: 0, bottom: 12, right: 0),
                metrics: Metrics = .portrait,
                mediaAspectMultiplier: CGFloat = 16.0 / 9.0
            ) {
                self.style = style
                self.bodyDisplay = bodyDisplay
                self.insets = insets
                self.metrics = metrics
                self.mediaAspectMultiplier = mediaAspectMultiplier
            }

            public static let `default` = Portrait()
            public static let portrait9x16 = Portrait()
            ...
            public func hash(into hasher: inout Hasher) {
                ...
                hasher.combine(insets.right)
                hasher.combine(metrics)
                hasher.combine(mediaAspectMultiplier)
            }
        }
