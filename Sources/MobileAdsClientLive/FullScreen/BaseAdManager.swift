...
protocol PaidEventCapable: AnyObject {
    ...
    /// Carries which network in the mediation stack actually filled the ad, which
    /// is the only place that name is available to us.
    ...
    @unchecked Sendable
{
    ...
    ///
    /// `featureID` and `slotRef` are empty at load time: a warm/pool/preload load
    /// happens before any show request exists, so the eventual consumer is unknown.
    /// The handler is re-attached with the real ones in `acquireForPresentation`.
    ...
    final func acquireForPresentation(
        ...
        let ad: AdType?
        if usesGoogle(adUnitID, keywords: keywords), let googleAd = googleSource.acquire(adUnitID) {
            logPool("show: GOOGLE preloaded · unit=\(adUnitID)")
            ad = googleAd
        } else {
            ad = await pooledSource.acquire(adUnitID, keywords: keywords, onColdLoad: onColdLoad)
        }

        // Bind revenue to the feature that is actually showing this ad. Several
        // features can share a unit, so the unit id alone can't attribute it, and
        // the load-time handler was attached before this request existed.
        if let capable = ad as? PaidEventCapable {
            attachPaidEventHandler(capable, adUnitID: adUnitID, featureID: featureID, slotRef: slotRef)
        }
        return ad
    }
    ...
    public final func showAd(
        ...
        guard
            let ad = await acquireForPresentation(
                adUnitID,
                keywords: keywords,
                featureID: featureID,
                slotRef: slotRef,
