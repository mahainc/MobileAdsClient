...
final class PooledAdSource<Ad: FullScreenPresentingAd & Sendable>: @unchecked Sendable {
    let pool: AdPool<Ad>
    ...
    /// Cache hit → return immediately. Miss → concurrent warms for the same
    /// key share one load: the first caller starts it — in its own
    /// unstructured task, so cancelling one caller can't cancel a load
    /// another caller is awaiting — with bounded retry, then stores + returns;
    /// every other caller just awaits that same task. Never evicts other
    /// variants. Returns nil only when all attempts fail.
    @discardableResult
    func warm(
        _ adUnitID: String,
        keywords: [String]
        ...
        let (loadTask, isJoining) = pool.joinOrBeginLoading(key) {
            Task<Ad?, Never> { await self.runLoad(adUnitID: adUnitID, keywords: keywords, key: key) }
        }
        if isJoining {
            log("warm: load already in flight · unit=\(adUnitID) · keywords=\(keywords) → joining in-flight load")
        }
        return await loadTask.value
    }

    /// The load itself: bounded retry with backoff, then store on success.
    /// Runs inside its own unstructured `Task` owned by `warm`, so every
    /// concurrent `warm` for `key` can await this one result without any of
    /// them being able to cancel it. Removes the pool's in-flight bookkeeping
    /// for `key` exactly once, when this finishes (success or final failure).
    private func runLoad(
        adUnitID: String,
        keywords: [String],
        key: AdPool<Ad>.CacheKey
    ) async -> Ad? {
