...
#if canImport(UIKit)
import Foundation
// The in-flight-load dictionary below stores `Task<Ad?, Never>`, and `Ad`
// (GoogleMobileAds' full-screen ad classes) isn't Sendable-audited — without
// `@preconcurrency` that's a hard "does not conform to Sendable" error, not
// just a warning.
...
final class AdPool<Ad: FullScreenPresentingAd & Sendable>: @unchecked Sendable {
    ...
    /// The load already in flight for each key, so a second concurrent `warm`
    /// for the same variant joins this task's result instead of issuing a
    /// duplicate network request (or, before this, returning nil outright).
    private var inFlightLoads: [CacheKey: Task<Ad?, Never>] = [:]
    ...
    /// Atomically joins the load already in flight for `cacheKey`, or starts
    /// one via `makeTask` and registers it as in-flight. `isJoining` tells the
    /// caller which happened (for logging) — either way it awaits the
    /// returned task. `makeTask` runs while `lock` is held, so it must not
    /// touch this pool's lock before its first `await` (build the task, let
    /// its first suspension be the actual network call).
    func joinOrBeginLoading(
        _ cacheKey: CacheKey,
        makeTask: () -> Task<Ad?, Never>
    ) -> (task: Task<Ad?, Never>, isJoining: Bool) {
        lock.lock()
        defer { lock.unlock() }
        if let existing = inFlightLoads[cacheKey] {
    ...
    /// Removes `cacheKey`'s in-flight load once it finishes (success or final
    /// failure), so the next `warm` for this key starts a fresh load instead
    /// of joining a completed one. Call exactly once, from the load task itself.
