...
public struct Native: TCAInitializableReducer, Sendable {
    ...
    public struct State: Identifiable, Sendable, Equatable {
        ...
        /// pre-loaded `NativeAd`, so the reducer's `.onAppear` short-circuits
        /// and the view renders the creative immediately. `adUnitID` and
        /// `adLoaderOptions` are left empty because no further load is issued.
        public init(
