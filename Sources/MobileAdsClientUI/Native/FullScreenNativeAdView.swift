...
//
//  The close control is NOT here. It belongs to `FullScreenNativeAdContainerView`,
//  which overlays it as a sibling so an unregistered control never sits inside this
//  `NativeAdView`'s hierarchy. This view only keeps its space clear.
//
...
extension FullScreenNativeAdView {
    private func setupViews() {
        ...
        NSLayoutConstraint.activate([
            ...
            // Never let the cluster grow up into the space the container's close
            // chrome occupies. That chrome is not in this hierarchy, so the space
            // is reserved from its published geometry rather than from a sibling.
            bottomCluster.topAnchor.constraint(
                greaterThanOrEqualTo: guide.topAnchor,
                constant: FullScreenCloseChromeLayout.reservedTopSpace
            ),
